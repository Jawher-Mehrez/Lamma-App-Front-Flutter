...
class RoomData2 extends StatefulWidget {
  final Map<String, dynamic>? roomData;
  final int? playerId;
  const RoomData2({Key? key, this.roomData,this.playerId}) : super(key: key);
  @override
  State<RoomData2> createState() => _RoomData2State();
}

class _RoomData2State extends State<RoomData2> {
  List<Player> players = [];
  String description = '';
  String startDate = '';
  String endDate = '';
  ...
  Future<void> _fetchStoredRoomId() async {
    ...
    if (_roomId != null) {
      ...
      _fetchRoomData(
          _roomId!); // Fetch the room data based on the stored room ID
    }
  }


  void _fetchRoomData(int roomId) async {
    final roomUrl = 'http://127.0.0.1:8000/api/rooms/$roomId';
    final playerUrl = 'http://127.0.0.1:8000/api/room-players/$roomId/join-player/12';
  ...
  void _leaveRoom() async {
    if (_roomId != null) {
      final url = 'http://127.0.0.1:8000/api/room-players/$_roomId/leave/7?_method=PUT';
      try {
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                Navigator.pushNamed(context, '/RoomUpdate2');
              },
            ),
          ],
        ),
